...
class VehicleTelemetryService
{
    ...
    /**
     * Get the latest data point reading for a specific vehicle and data point type.
     *
     ...
     */
    public function getLatestReading(Vehicle $vehicle, int $dataPointTypeId): mixed
    {
        ...
        $cacheKey = "vehicle_telemetry:{$vehicle->id}:latest:{$dataPointType->id}";

        return Cache::remember($cacheKey, self::CACHE_TTL, function () use ($vehicle, $dataPointType) {
            if ($dataPointType->category === 'ATOMIC') {
                $reading = DeviceDataPoint::where('vehicle_id', $vehicle->id)
                    ->where('data_point_type_id', $dataPointType->id)
                    ->orderByDesc('recorded_at')
                    ->first();
                return $reading ? $reading->value : null;
            } elseif ($dataPointType->category === 'COMPOSITE') {
                return $this->calculateCompositeValue($vehicle, $dataPointType);
            }
            return null;
        });
    }
    ...
    /**
     * Get data point readings for a specific vehicle, type, and period.
     *
     * @param Vehicle $vehicle
     ...
     */
    public function getReadingsForPeriod(Vehicle $vehicle, int $dataPointTypeId, Carbon $startTime, Carbon $endTime): \Illuminate\Support\Collection
    {
        $dataPointType = $this->resolveDataPointType($dataPointTypeId);
        ...
        return DeviceDataPoint::where('vehicle_id', $vehicle->id)
            ->where('data_point_type_id', $dataPointType->id)
    ...
    /**
     * Get aggregated data for a specific vehicle, type, and period.
    ...
    public function getAggregatedReadings(
        ...
        $query = DeviceDataPoint::where('vehicle_id', $vehicle->id)
            ->where('data_point_type_id', $dataPointType->id)
            ->whereBetween('recorded_at', [$startTime, $endTime]);
        ...
        if (!in_array($safeAggregationFunction, $allowedAggregations)) {
            Log::error('Invalid aggregation function requested.', ['function' => $aggregationFunction, 'vehicle_id' => $vehicle->id]);
            return [];
        }
        ...
        if ($timeBucket) {
            ...
            if (!preg_match('/^\\d+\\s+(second|minute|hour|day|week|month|year)s?$/', $timeBucket)) {
                Log::error('Invalid time bucket format for aggregation.', ['bucket' => $timeBucket, 'vehicle_id' => $vehicle->id]);
                return [];
            }
     ...
     */
    protected function calculateCompositeValue(Vehicle $vehicle, DataPointType $compositeType): mixed
    {
        ...
        try {
                ...
                switch ($logic) {
                    case 'GET_LATEST_FROM_SOURCE':
                        if (isset($step['source_data_point_type_id'])) {
                            $currentValue = $this->getLatestReading($vehicle, $step['source_data_point_type_id']);
                    ...
                    case 'PRIORITY_PICK':
                        if (isset($step['sources']) && is_array($step['sources'])) {
                            ...
                            foreach ($sortedSources as $sourceConfig) {
                                ...
                                $sourceValue = $this->getLatestReading($vehicle, $sourceConfig['data_point_type_id']);
                                if ($sourceValue !== null) {
            ...
            Log::error('Error calculating composite value for VehicleTelemetryService', [
                'vehicle_id' => $vehicle->id,
                'composite_type_id' => $compositeType->id,
