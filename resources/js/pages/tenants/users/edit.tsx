...
export default function TenantUserEdit({ tenant, user }: TenantUserEditProps) {
    ...
    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        patch(route('tenants.users.update', [tenant.id, user.id]));
    };
                ...
                <div className="space-y-6">
                    ...
                    <Card>
                        <CardHeader className="pb-4">
                            <CardTitle>{__('tenant_users.edit.form_title')}</CardTitle>
                            <CardDescription>{__('tenant_users.edit.form_description', { name: user.name })}</CardDescription>
                        </CardHeader>
                        <form onSubmit={submit}>
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        <div>
                                            ...
                                            <p className="mt-1 text-sm text-neutral-500">
                                                {__('common.leave_blank_to_keep_unchanged')}
                                            </p>
                            ...
                            <CardFooter className="flex items-center justify-between space-x-2 pt-6">
                                <Transition
                                    ...
                                    leaveTo="opacity-0"
                                >
                                    <p className="text-sm text-green-600">{__('tenant_users.messages.updated')}</p>
                                </Transition>

                                <div className="flex justify-end">
                                    <Button type="submit" disabled={processing}>
                                        {__('tenant_users.actions.save')}
                                    </Button>
                                </div>
                            </CardFooter>
                        </form>
